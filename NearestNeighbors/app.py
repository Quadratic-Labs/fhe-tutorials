...
if "evaluation_key" not in st.session_state:
    with c1:
        if st.button("Generate keys"):
            with st.spinner("Generating keys"):
                client.keys.load_if_exists_generate_and_save_otherwise(
                    config.keys_filepath
                )
                st.session_state["evaluation_key"] = client.evaluation_keys.serialize()
                add_to_client_side(
                    "Encryption/decryption keys and evaluation keys are generated."
                )
                add_to_client_side("The evaluation key is sent to the server.")
                add_to_server_side(
                    f"Evaluation key: {display_encrypted(st.session_state['evaluation_key'])}"
                )
            st.rerun()
    ...
    if "position" not in st.session_state:
        ...
        if st_map.get("last_clicked"):
            position = Point(
                st_map.get("last_clicked")["lng"], st_map.get("last_clicked")["lat"]
            )
            add_to_client_side(
                f"Selected starting point is: ({position.x}, {position.y})"
            )
        ...
        with c1:
            if st.button("Encrypt and send inputs"):
                with st.spinner("Encrypting inputs"):
                    ...
                    encrypted_position = (
                        encrypted_x.serialize(),
                        encrypted_y.serialize(),
                    )
                    add_to_server_side(
                        f"Received starting point: ({display_encrypted(encrypted_position[0])},{display_encrypted(encrypted_position[1])})"
                    )
                    st.session_state["encrypted_position"] = encrypted_position
                st.rerun()

    if (
        "encrypted_position" in st.session_state
        and "encrypted_results" not in st.session_state
    ):
        with c3:
            encrypted_x = st.session_state["encrypted_position"][0]
            encrypted_y = st.session_state["encrypted_position"][1]

            if st.button(f"Find {config.number_of_neighbors} nearest restaurants"):
                with st.spinner("Computing"):
                    st.write("Please wait, it may take few minutes.")
                    ...
                    add_to_server_side(
                        f"The encrypted result {display_encrypted(ser_res)} is sent to the client."
                    )
                    add_to_client_side(
                        f"The received encrypted result is: {display_encrypted(ser_res)}."
                    )
                    st.session_state["encrypted_results"] = ser_res
                st.rerun()

    if (
        "encrypted_results" in st.session_state
        and "decrypted_result" not in st.session_state
    ):
